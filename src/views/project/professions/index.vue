...
<script>
  import { findAllProfession, deleteProfession } from "@/api/professions";
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(row) {
        if (row.id) {
          this.$baseConfirm("你确定要删除当前项吗", null, async () => {
            const { msg } = await deleteProfession({ ids: row.id });
          ...
          if (this.selectRows.length > 0) {
            ...
            this.$baseConfirm("你确定要删除选中项吗", null, async () => {
              const { msg } = await deleteProfession({ ids });
              this.$baseMessage(msg, "success");
      ...
      async fetchData(loading = true) {
        ...
        const {
          data: {
            professionList: { list, total },
          },
        } = await findAllProfession(this.queryForm);
